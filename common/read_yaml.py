...
class ReadYaml:
    ...
    def __init__(self, file_path=None):
        if file_path is None:
            parameter = open(r"..\config\publicParameter.yml", mode="r", encoding="utf-8")
        else:
    ...
    def read_request_parameter(self, path, environment=None):
        ...
        # 将请求的域名+请求路径+请求的参数拼接
        link = domain_name + path + "?" + url_enc
        return link


if __name__ == "__main__":
    ry = ReadYaml()
    url = ry.read_request_parameter("/app/hpjy/getHpjyNewUserGuide", "现网")
    print(url)
